...
export default {
    ...
    methods:{
        handleAvatarSuccess(res, file) {
            console.log('handleAvatarSuccess ===> ', typeof this.tmpPaths, this.tmpPaths, res)
            if (this.multiple) {
                this.tmpPaths.push(...res.paths)
            } else {
                this.tmpPaths = res.paths[0]
            }
            console.log('handleAvatarSuccess 22 ===>>', this.tmpPaths)
        ...
        beforeAvatarUpload(file) {
            console.log('beforeAvatarUpload fileType ==> ', file.type)
